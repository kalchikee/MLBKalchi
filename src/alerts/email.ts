...
/**
 * Send the EOD bet results + P&L summary via Resend email.
 * Mirrors the content of sendEODSummaryAlert() (Discord) but as an HTML email.
 * No-op if RESEND_API_KEY or RESEND_TO are not set.
 */
export async function sendEODSummaryEmail(
  ...
  let betsHtml = '';
  if (bets.length === 0) {
    betsHtml = '<p style="color:#8b949e">No bets were placed today.</p>';
  } else {
    let rows = '';
    for (const b of bets) {
    ...
    betsHtml = `
      <table>
        <thead>
          <tr><th>Matchup</th><th>Entry</th><th>Result</th></tr>
        </thead>
        <tbody>${rows}</tbody>
      </table>
    `;
  ...
  const html = `
    <!DOCTYPE html><html><head><style>${baseStyle}</style></head><body>
    <div class="container">
      <h1>📋 MLB Oracle EOD${mode}</h1>
      <p style="color:#8b949e">${date} &nbsp;·&nbsp; ${bets.length} bet${bets.length !== 1 ? 's' : ''}</p>
      <div class="card">
        <h3>Bet Results</h3>
        ${betsHtml}
      </div>
      <div class="card">
        ...
        <table>
          ...
          <tr><td>Record</td><td>${summary.wins}W &nbsp; ${summary.losses}L${summary.open > 0 ? ` &nbsp; ${summary.open} open` : ''}</td></tr>
          <tr><td>Total Wagered</td><td>$${summary.totalCost.toFixed(2)}</td></tr>
          <tr><td>Net P&amp;L</td><td class="${summary.totalPnl >= 0 ? 'correct' : 'wrong'}"><strong>${pnlStr}</strong></td></tr>
          <tr><td>ROI</td><td>${roi.toFixed(1)}%</td></tr>
        </table>
      </div>
      <div class="footer">MLB Oracle v4.0.0${mode} &nbsp;·&nbsp; ${new Date().toISOString()}</div>
    </div>
    </body></html>
  `;

  await sendEmail(`MLB Oracle EOD — ${date} — ${pnlStr}`, html);
}
